...
void Snake::Init(const sf::Texture &texture,float StartPointY)
{
    float x = 16.f;
    for (auto &piece : m_body)
    {
        piece.setTexture(texture);
        piece.setPosition({x, StartPointY});
        x += 16.f;
    }
}
...
bool Snake::IsIntesectingWithOther(Snake s) const {
    bool flag = false;

    for (auto piece = s.m_body.begin(); piece != s.m_body.end(); ++piece)
    {
            flag = IsOn(*piece);
            if (flag)
            {
