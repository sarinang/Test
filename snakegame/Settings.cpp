...
void Settings::Init()
{
    ...
    // Exit Button
    m_hardness.setFont(m_context->m_assets->GetFont(MAIN_FONT));
    if(m_context->isHard){
        m_hardness.setString("Difficulty : Hard");
    } else{
        m_hardness.setString("Difficulty : Normal");
    }
    m_hardness.setOrigin(m_hardness.getLocalBounds().width / 2,
                         m_hardness.getLocalBounds().height / 2);
    m_hardness.setPosition(m_context->m_window->getSize().x / 2 - 10.f,
                                m_context->m_window->getSize().y / 2 + 25.f);
    m_hardness.setCharacterSize(CHARACTER_SIZE);
